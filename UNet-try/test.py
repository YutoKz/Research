...
plt.figure()
plt.imshow(data["img"][num,:,:,:].permute(1, 2, 0))
plt.title("original_image")
plt.axis("off")
plt.savefig("/result_png/original.png")
...
for i in range(2):
  for j, cl in enumerate(classes):
    if i == 0:
      ax[i,j].imshow(pred[num,:,:,j])
      ax[i,j].set_title(f"pred_{cl}")
      ax[i,j].axis("off")
    else:
      ax[i,j].imshow(data["label"][num,j,:,:])    
      ax[i,j].set_title(f"label_{cl}")
      ax[i,j].axis("off")
plt.savefig("/result_png/pred.png")
